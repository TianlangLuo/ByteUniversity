...
let App = {
  ...
  sessionsBeforeTen: 0, // sessions started before 10am
  onTimeReviews: 0,     // reviews done on scheduled date
  firstLearnDate: null,
...
function loadState() {
  try {
    ...
    App.sr              = s.sr               || {};
    App.xp              = s.xp               || 0;
    App.totalXpEarned   = s.totalXpEarned    || 0;
    App.xpSpent         = s.xpSpent          || 0;
    App.equippedAvatar  = s.equippedAvatar   || 'default';
    App.unlockedAvatars = s.unlockedAvatars  || ['default'];
    App.badges          = s.badges           || [];
    App.sessionsBeforeTen = s.sessionsBeforeTen || 0;
    App.onTimeReviews   = s.onTimeReviews    || 0;
    App.firstLearnDate  = s.firstLearnDate   || null;
    App.lastActivityDate= s.lastActivityDate || null;
    App.streakDays      = s.streakDays       || 0;
...
function saveState() {
  try {
    localStorage.setItem(STATE_KEY, JSON.stringify({
      ...
      sr:               App.sr,
      xp:               App.xp,
      totalXpEarned:    App.totalXpEarned,
      xpSpent:          App.xpSpent,
      equippedAvatar:   App.equippedAvatar,
      unlockedAvatars:  App.unlockedAvatars,
      badges:           App.badges,
      sessionsBeforeTen:App.sessionsBeforeTen,
      onTimeReviews:    App.onTimeReviews,
      firstLearnDate:   App.firstLearnDate,
...
function awardXP(amount, label) {
  App.xp += amount;
  App.totalXpEarned += amount;
  if (!App.firstLearnDate) App.firstLearnDate = todayStr();
  updateStreak();
  saveState();
  updateXPBar();
  checkBadges();
  // Float up animation
  showXPPop('+' + amount + ' XP' + (label ? ' · ' + label : ''));
}

function spendXP(amount) {
  if (App.xp < amount) return false;
  App.xp -= amount;
  App.xpSpent += amount;
  saveState();
  updateXPBar();
...
function updateXPBar() {
  ...
  const progress = App.totalXpEarned % 200;
  el.style.width = (progress / 200 * 100) + '%';
  if (lbl) lbl.textContent = App.xp + ' XP';
...
function showXPPop(text) {
  ...
  setTimeout(() => pop.classList.add('rise'), 10);
  setTimeout(() => pop.remove(), 1800);
}

function updateStreak() {
  const today = todayStr();
  if (App.lastActivityDate === today) return;
  const yesterday = new Date();
  yesterday.setDate(yesterday.getDate() - 1);
  const yStr = yesterday.toISOString().slice(0, 10);
  if (App.lastActivityDate === yStr) {
    App.streakDays++;
  } else if (App.lastActivityDate !== today) {
    App.streakDays = 1;
  }
  App.lastActivityDate = today;
}

/* ── BADGES ── */
const BADGES = [
  { id:'first_lesson',    icon:'🎓', name:'First Steps',        desc:'Complete your first subtopic',             check:() => App.learned.size >= 1 },
  { id:'all_topics',      icon:'🏆', name:'Master of All',      desc:'Complete all 7 subtopics',                 check:() => App.learned.size >= 7 },
  { id:'early_bird',      icon:'🌅', name:'Early Bird',         desc:'Start a session before 10am',              check:() => App.sessionsBeforeTen >= 1 },
  { id:'early_bird_3',    icon:'☀️',  name:'Rise & Grind',       desc:'Start 3 sessions before 10am',             check:() => App.sessionsBeforeTen >= 3 },
  { id:'xp_100',          icon:'⚡', name:'Charged Up',         desc:'Earn 100 total XP',                        check:() => App.totalXpEarned >= 100 },
  { id:'xp_500',          icon:'💎', name:'XP Hoarder',         desc:'Earn 500 total XP',                        check:() => App.totalXpEarned >= 500 },
  { id:'xp_1000',         icon:'👑', name:'XP Royalty',         desc:'Earn 1000 total XP',                       check:() => App.totalXpEarned >= 1000 },
  { id:'xp_2500',         icon:'🌟', name:'Legendary',          desc:'Earn 2500 total XP',                       check:() => App.totalXpEarned >= 2500 },
  { id:'streak_3',        icon:'🔥', name:'On a Roll',          desc:'Study 3 days in a row',                    check:() => App.streakDays >= 3 },
  { id:'streak_7',        icon:'🌋', name:'Week Warrior',       desc:'Study 7 days in a row',                    check:() => App.streakDays >= 7 },
  { id:'on_time_1',       icon:'📅', name:'Punctual',           desc:'Complete a review on its scheduled date',  check:() => App.onTimeReviews >= 1 },
  { id:'on_time_5',       icon:'🗓️',  name:'Disciplined',        desc:'Complete 5 reviews on their scheduled dates', check:() => App.onTimeReviews >= 5 },
  { id:'shopper',         icon:'🛍️',  name:'Shopaholic',         desc:'Buy your first avatar',                    check:() => App.unlockedAvatars.length >= 2 },
  { id:'collector',       icon:'🎭', name:'Collector',          desc:'Unlock 5 different avatars',               check:() => App.unlockedAvatars.length >= 5 },
  { id:'perfect_quiz',    icon:'💯', name:'Perfectionist',      desc:'Score 100% on a quiz',                     check:() => (App._lastQuizPct || 0) >= 100 },
  { id:'quiz_master',     icon:'🧙', name:'Quiz Master',        desc:'Score 80%+ on quizzes for all 7 topics',   check:() => Object.keys(App.sr).length >= 7 },
  { id:'night_owl',       icon:'🦉', name:'Night Owl',          desc:'Start a session after 10pm',               check:() => (App._lastHour || 0) >= 22 },
  { id:'speed_run',       icon:'⚡', name:'Speed Runner',       desc:'Complete a subtopic in under 3 minutes',   check:() => App._speedRun === true },
];

function checkBadges() {
  let newBadges = [];
  BADGES.forEach(b => {
    if (!App.badges.includes(b.id) && b.check()) {
      App.badges.push(b.id);
      newBadges.push(b);
    }
  });
  if (newBadges.length) {
    ...
    newBadges.forEach(b => {
      setTimeout(() => {
        toast(`${b.icon} Badge unlocked: ${b.name}!`, 'ye');
      }, 400);
    });
    if (document.getElementById('page-profile') &&
        document.getElementById('page-profile').classList.contains('on')) {
      renderProfile();
    }
  }
}
...
/* ── AVATARS ── */
const AVATARS = [
  { id:'default',   name:'Pixel Bot',     cost:0,    emoji:'🤖', bg:'#6366f1', desc:'Your starter avatar' },
  { id:'wizard',    name:'Code Wizard',   cost:100,  emoji:'🧙', bg:'#7c3aed', desc:'Master of algorithms' },
  { id:'astronaut', name:'Data Naut',     cost:150,  emoji:'👨‍🚀', bg:'#0891b2', desc:'Exploring the cloud' },
  { id:'ninja',     name:'Cyber Ninja',   cost:200,  emoji:'🥷', bg:'#be185d', desc:'Silent and stealthy' },
  { id:'robot',     name:'AI Unit X9',    cost:250,  emoji:'🦾', bg:'#059669', desc:'Powered by machine learning' },
  { id:'ghost',     name:'Phantom Hacker',cost:300,  emoji:'👻', bg:'#374151', desc:'White-hat legend' },
  { id:'dragon',    name:'Firewall Drake',cost:400,  emoji:'🐉', bg:'#b45309', desc:'Breathes encryption' },
  { id:'alien',     name:'Extraterrestrial',cost:500,emoji:'👾', bg:'#4f46e5', desc:'Out of this world' },
  { id:'unicorn',   name:'Unicorn Dev',   cost:750,  emoji:'🦄', bg:'#db2777', desc:'Mythically productive' },
  { id:'crown',     name:'XP Royalty',    cost:1000, emoji:'👑', bg:'#d97706', desc:'The ultimate flex' },
];

function renderAvatar(id, size = 36) {
  const av = AVATARS.find(a => a.id === id) || AVATARS[0];
  return `<div class="av-badge" style="width:${size}px;height:${size}px;background:${av.bg};border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:${Math.round(size*0.5)}px;flex-shrink:0;">${av.emoji}</div>`;
}
...
function markReviewed(topicId) {
  ...
  // Check if reviewed on scheduled date
  if (sr.nextReview && sr.nextReview === todayStr()) {
    App.onTimeReviews++;
    awardXP(25, 'On-time review!');
  }
